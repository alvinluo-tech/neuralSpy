import { detectWinnerByRole, isWhiteboardRole, WHITEBOARD_GUESS_PENDING_MARKER } from "@/lib/gameEngine";
...
export type SettleVoteRoundInput = {
  roomId: string;
...
export type SettleVoteRoundResponse = {
  status: number;
  payload: Record<string, unknown>;
};

const ok = (payload: Record<string, unknown>): SettleVoteRoundResponse => ({
  status: 200,
  payload,
});

const fail = (status: number, message: string): SettleVoteRoundResponse => ({
  status,
...
export async function settleVoteRound(input: SettleVoteRoundInput): Promise<SettleVoteRoundResponse> {
  try {
    ...
    if (roomRes.error) {
      return fail(500, roomRes.error.message);
    }
    ...
    if (room.status !== "voting") {
      return ok({ ok: true, action: "noop", reason: "not-voting" });
    }

    if (
      typeof input.expectedRound === "number" &&
      typeof input.expectedVoteRound === "number" &&
      (input.expectedRound !== room.round_number || input.expectedVoteRound !== room.vote_round)
    ) {
      return ok({ ok: true, action: "noop", reason: "stale-client" });
    }
    ...
    if (playersRes.error) {
      return fail(500, playersRes.error.message);
    }
    ...
    if (votesRes.error) {
      return fail(500, votesRes.error.message);
    }
    ...
    const deadlineReached =
      !!room.vote_deadline_at && nowMs >= Date.parse(room.vote_deadline_at);
    const forceSettle = input.force === true;
    ...
    if (countedVotes.length === 0) {
      ...
      if (pendingRes.error) {
        return fail(500, pendingRes.error.message);
      }
      ...
      return ok({ ok: true, action: "revote-no-votes-pending" });
    }
    ...
    if (candidates.length > 1) {
      ...
      if (tieRes.error) {
        return fail(500, tieRes.error.message);
      }
      ...
      return ok({ ok: true, action: "revote-tie-pending", candidates });
    }

    const eliminatedId = candidates[0];
    const eliminatedPlayer = players.find((player) => player.id === eliminatedId);
    if (!eliminatedPlayer) {
      return fail(500, "Eliminated player not found.");
    }
    ...
    if (eliminateRes.error) {
      return fail(500, eliminateRes.error.message);
    }

    if (!eliminateRes.data) {
      return ok({ ok: true, action: "noop", reason: "already-eliminated" });
    }
    ...
    if (isWhiteboardRole(eliminatedPlayer) && players.length >= 4) {
      ...
      if (whiteboardPendingRes.error) {
        return fail(500, whiteboardPendingRes.error.message);
      }
    ...
    if (winner) {
      ...
      if (finishRes.error) {
        return fail(500, finishRes.error.message);
      }
    ...
    if (continueRes.error) {
      return fail(500, continueRes.error.message);
    }
